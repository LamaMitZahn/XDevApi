package de.ruben.xdevapi.custom.hologram;
...
public class XHologram implements HologramInterface {
    ...
    public XHologram(XDevApi plugin, Location location, boolean global) {
    ...
    public void spawnHologram() {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, this::updateHologram);

        if (consumer == null || updateInterval <= 0)
            return;
        this.hologramUpdater = Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, () -> {
            if (isDeleted())
        ...
        Bukkit.getScheduler().runTaskLater(plugin, this::delete, lifeTimeInTicks);
    }
